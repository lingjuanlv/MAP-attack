...
%       nlfunc = @poly7;
  ...
  methods
    % Constructor
    function this = PrivacyTest_DKE(scheme, attack, fRecovOut)      
      this.scheme = scheme;
      this.attack = attack;
    ...
    function recovrate = do(this, X, deltaw)
      ...
      nOutliers = round(m*this.pcOutliers);      % number of leaked records
      nLeaked = round(m*this.pcLeaked);          % number of outliers to generate
      nRecovd = round(m*this.pcRecovd);          % number of records to recover
      recovrate = zeros(length(this.epsilons),2); % column 1 has recovery rate defined by Sang et al.
                                                  % column 2 has recovery rate defined by me 
